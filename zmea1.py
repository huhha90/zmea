...
class SnakeGame:
    ...
    def show_mods_menu(self):
            ...
            mod_names = self.loaded_mods if hasattr(self, "loaded_mods") else ["(немає активних модів)"]
            button_rects = self._create_button_rects(mod_names + [self.get_text("back")])

            while active:
                self.screen.fill(BG_COLOR)
                title = self.font.render("Активні моди", True, TEXT_COLOR)
                title_rect = title.get_rect(center=(self.width // 2, 100))
                self.screen.blit(title, title_rect)

                mouse_pos = pygame.mouse.get_pos()
                self._draw_buttons(button_rects, mouse_pos)
        ...
        for folder in os.listdir(mods_dir):
            ...
            with open(config_path, "r", encoding="utf-8") as f:
                try:
                    ...
                    self.apply_mod(mod_path, mod_config)
                    self.loaded_mods.append(mod_config.get("name", "Unnamed mod"))
                except Exception as e:
                    print(f"Помилка завантаження моду {folder}: {e}")
